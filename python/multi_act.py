...
parser.add_argument(
    "--self_contained",
    action="store_true",
    help="Copy all input files into output directory, so the whole output is self-contained and doesn't rely on the original input",
)
parser.add_argument(
    "--zip",
    action="store_true",
    help="Make a zip archive of the output files, called <outdir>.zip. Forces --self_contained",
)
parser.add_argument(
    "--blast_ops",
    help="blastall options [%(default)s]",
    default="-p blastn -m 8 -F F -e 0.01 -b 10000 -v 10000",
)
parser.add_argument(
    "--nucmer_ops",
    help="nucmer or promer options [promer:--maxmatch. nucmer: --maxmatch --nosimplify]",
)
parser.add_argument("--no_delta_filter", action="store_true")
parser.add_argument(
    "--no_act",
    action="store_true",
    help="Do not start act, just make comparison files etc",
)
parser.add_argument(
    "--delta_ops", help="delta-filter options [%(default)s]", default="-m"
)
...
def compare_with_blast(qry, ref, ops, outfile):
    subprocess.check_output(
        "formatdb -l " + os.path.join(ops.outdir, ".formatdb.log") + " -p F -i " + ref,
        shell=True,
    )
    cmd = " ".join(["blastall", ops.blast_ops, "-d", ref, "-i", qry, "-o", outfile])
...
def compare_with_nucmer(qry, ref, ops, outfile):
    ...
    cmd = " ".join(["show-coords -dTlroH", filtered_file, ">", coords_file])
    print("cmd:", cmd)
    pyfastaq.utils.syscall(cmd)
    ...
    pymummer.coords_file.convert_to_msp_crunch(
        coords_file, outfile, qry + ".fai", ref + ".fai"
    )
