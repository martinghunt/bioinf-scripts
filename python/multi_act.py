...
parser = argparse.ArgumentParser(
    ...
    usage = '%(prog)s [options] <blast|nucmer> <outprefix> <file1.fa> <file2.fa>  [<file3.fa ...]')
parser.add_argument('--blast_ops', help='blastall options [%(default)s]', default='-p blastn -m 8 -F F -e 0.01 -b 10000 -v 10000')
parser.add_argument('--nucmer_ops', help='nucmer options [%(default)s]', default='--nosimplify --maxmatch')
parser.add_argument('--delta_ops', help='delta-filter options [%(default)s]', default='-m')
parser.add_argument('aln_tool', help='blast or nucmer')
...
def compare_with_blast(qry, ref, ops, outfile):
    ...
    subprocess.check_output('formatdb -l ' + options.outprefix + '.formatdb.log -p F -i ' + ref, shell=True)
    cmd = ' '.join([
...
def compare_with_nucmer(qry, ref, ops, outfile):
    ...
    cmd = 'mummer2crunch.py ' + coords_file + ' ' + outfile
    print('cmd:', cmd)
    pyfastaq.utils.syscall(cmd)
